...
loop:
	...
	fprintf(stderr, "0: Total Numbers\n");
	...
	if (block != NULL) {
		...
		fprintf(stderr, "1> Get free block from free list\n");
	...
	if (freed) {
		fprintf(stderr, "2> Get free block by LRU scan\n");
	...
	fprintf(stderr, "3> Get free block by single page flush\n");
	goto loop;
}
